...
for channelNr = activeChnList
    ...
    navBitsSamples = trackResults(channelNr).I_P(subFrameStart(channelNr) : ...
                               subFrameStart(channelNr) + (15000 * 2) -1)';
    ...
    %=== Decode ephemerides and TOW of the first sub-frame ================
        [eph(trackResults(channelNr).PRN), SOW] = ephemeris_D2(navBits');
    else
        navBitsSamples = reshape(navBitsSamples, ...
                             20, (size(navBitsSamples, 1) / 20));
        navBits = NHcode * navBitsSamples;
        navBits = (navBits > 0);
        [eph(trackResults(channelNr).PRN), SOW] = ephemeris_D1(navBits');
    end
    
    %--- Exclude satellite if it does not have the necessary nav data -----
    if (isempty(eph(trackResults(channelNr).PRN).health) || ...
        isempty(eph(trackResults(channelNr).PRN).IDOT)) %|| ...
        %(eph(trackResults(channelNr).PRN).health))
...
for currMeasNr = 1:fix((settings.msToProcess - max(subFrameStart)) / ...
    ...
    activeChnList = intersect(readyChnList,find(satElev >= settings.elevationMask));
