...
[tau1carr, tau2carr] = calcLoopCoef(settings.pllNoiseBandwidth, ...
                                    ...
                                    0.25);
% [tau1carrPull, tau2carrPull] = calcLoopCoef(110, 0.707, 0.25);
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0 &&[channel(channelNr).status] ~= '-')
        ...
        caCode = generaterangcode(channel(channelNr).PRN);
...
%% Find PLL error and update carrier NCO ----------------------------------
            ...
            loopFilterOut = (tau2carr/tau1carr) * ...
            ...
            carrFreq = carrFreq + loopFilterOut;
%             carrFreqBasis = carrFreqBasis + (carrError - oldCarrError)/(0.001*2*pi);
            trackResults(channelNr).carrFreq(loopCnt) = carrFreq;
            oldCarrNco   = carrNco;
            oldCarrError = carrError;
%% Find DLL error and update code NCO -------------------------------------
            
            codeError = 0.5*(sqrt(I_E^2 + Q_E^2) - sqrt(I_L^2 + Q_L^2)) / ...
                (sqrt(I_E^2 + Q_E^2) + sqrt(I_L^2 + Q_L^2)) ;
            
            % Implement code loop filter and generate NCO command
                
            codeNco = oldCodeNco + (tau2code/tau1code) * ...
                (codeError - oldCodeError) + codeError * (PDIcode/tau1code)-loopFilterOut/763;
            oldCodeNco   = codeNco;
...
%% Record various measures to show in postprocessing ----------------------
            ...
            trackResults(channelNr).pllDiscrFilt(loopCnt)   = loopFilterOut;
